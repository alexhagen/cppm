...
everytime I have to use one of the complex ``c++`` objects, or - even
worse - use the old non-semantic ``c`` APIs for things, I try to write
...
Installation is easy. Just clone, copy into ``/usr/inclue/c++``, and go.
...
There's a bunch of different interfaces defined here, categorized
roughly as *memory objects*, *tcp objects*, *udp objects*, and *signals
and timing*. Specific documentation on all of those interfaces are
included in the readmes folder.
