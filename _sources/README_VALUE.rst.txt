The AH Value class is a simple, encapsulated c++ class to help track
...
checking if something is over a certain value every loop).

Installation
^^^^^^^^^^^^

Installation of the ah\_value class is simple. Place the ah\_value.h
...
Lets say we want to create a program that indicates whether a motor is
up to speed, and thereafter, how much it has varied from that speed. We
would write a notifier class, as below.
